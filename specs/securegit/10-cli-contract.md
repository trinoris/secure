...
| `securegit protect [<pattern>…]` | Add patterns to `.gitattributes` with `filter`, `diff`, `merge` and `-text`, keeping `.gitattributes -filter -diff -text` and `.securegit/** -filter -diff -text` (in that order) as the two permanent trailing lines. No pattern given protects `DEFAULT_PROTECT_PATTERNS` (`src/install.ts`) instead — `**`, secure by default, so a file added later never ships as plaintext just because nobody named it in advance — and automatically excludes `DEFAULT_PROTECT_EXCLUSIONS` (`.github/workflows/**`, since GitHub Actions cannot parse an encrypted workflow file) via `exclude`; the confirmation printed afterward names exactly what was applied and excluded. |
| `securegit unprotect <pattern>…` | Remove patterns from `.gitattributes` only — `.gitignore`'s residue entries are left alone. Warns that already-committed blobs stay encrypted until re-committed. A pattern that was never protected is a silent no-op (exit 0), and doesn't touch the file. Undoes one earlier `protect <pattern>` call specifically — for a pattern still covered by something broader (like the `**` default), use `exclude` instead. |
| `securegit exclude <pattern>…` | Write `<pattern> -filter -diff -text` before the two permanent trailing lines, guaranteeing the pattern is never filtered regardless of what broader positive pattern would otherwise have matched it (Git's own last-match-wins attribute resolution). Idempotent. The escape hatch for the `**` default — `unprotect` can't carve an exception out of a pattern it didn't itself add. |
...
| `securegit protect` with no pattern protects `**` and excludes `.github/workflows/**`, printing both | `src/cli.test.ts` | — | ✅ |
| `securegit exclude <pattern>` carves a real plaintext exception out of a catch-all `protect` pattern, proved against real `git check-attr` | `src/install.test.ts`, `src/cli.test.ts` | — | ✅ |
| `.gitattributes` itself is never filtered, even under a catch-all protect pattern (`GITATTRIBUTES_EXCLUSION_LINE`) | `src/install.test.ts` | — | ✅ |
