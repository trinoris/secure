...
| T3/T4/T5: a commit signed by a fingerprint not on the recipient list is rejected, regardless of attack shape | [chaos sandbox orchestrator](../chaotests/03-orchestrator.md)'s merge review and its `pre-receive` hook | — | ✅ built and confirmed on real GitHub Actions infrastructure ([run 34022273040](https://github.com/trinoris/secure/actions/runs/34022273040)): every `advance`-tier leg (direct-master, working-branch, pr-gated) held 0 violations; the matching `basic`-tier legs (no signing) leaked, confirming the check is what closes the gap — see that spec's own "Since then" note for the fixes this took |
