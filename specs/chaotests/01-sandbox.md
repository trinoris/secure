...
| bad-agent | Hostile AI agent | **Since then.** code-agent's adversarial counterpart: its own container, its own clone, ordinary push access only — a second, fully independent instance of `chaos/agents/attacker.mjs` (already generic over `SANDBOX_ROLE`, reused as-is, no code changes) running the exact same [16](../securegit/16-adversarial-integrity.md) catalogue as chaos-5, concurrently and never coordinated with it. Models a compromised or malicious AI agent, not a fundamentally different attack shape — the catalogue doesn't change just because the pusher is an agent instead of a human. |
