...
| code-agent | AI coding agent (actor-4) | **Since then.** Identical loop to actor-1/actor-2 (`collaboratorRound()` in `chaos/actors/driver.mjs` is already keyed by `SANDBOX_ROLE`, no branching added) — models an AI coding agent committing like any other legitimate collaborator: same protections, and, once `SANDBOX_SIGNING=advance` is on, the same real signing-identity registration (`COLLABORATOR_ROLES` there now lists all three ordinary roles). Deliberately no special-casing anywhere — the whole point is that this project's protections don't need to know or care whether a human or an agent produced a given commit. |
...
| bad-agent | Hijacked trusted AI agent | **Superseded — see [04-agent-threat-model.md](04-agent-threat-model.md).** No longer chaos-5's shape at all: `bad-agent` is a genuine `COLLABORATOR_ROLES` member in `chaos/actors/driver.mjs`, listed here (rather than in the legitimate-actors table above) only because most of its externally-visible behavior is attack traffic. It clones, registers a real signing identity exactly like code-agent, and runs the ordinary collaborator loop on most rounds — but on a fraction of them (`hijackedAgentRound()`), it instead produces one *validly signed* commit implementing PI1 (attribute downgrade), PI2 (blob relocation to an unprotected path, landing as real plaintext since its own working tree already holds the decrypted content), PI3 (direct plaintext exfiltration to an unprotected path — the one shape nothing in this project catches), or PI4 (a hostile recipient, added by an already-trusted signer). Models a legitimate, already-registered agent hijacked by a prompt injection, not an outsider — the entire point being that the signing check (which fully stops chaos-5) has nothing to say about any of these four. |
