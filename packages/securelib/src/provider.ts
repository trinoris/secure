...
/**
 * What a caller needs from an unlocked keyring/session. Injected rather
 * than imported by consumers like a Git filter, so their own contract is
 * testable without a real keyring, session cache, or key provider — the
 * real keyring implements exactly this shape. Lives here, not in any one
 * consumer, because it's what *this port* produces once a provider has
 * successfully unwrapped a generation — nothing about it is specific to
 * Git or any other consumer.
 */
export interface KeySource {
  /** The generation a caller should encrypt under. `null` when locked. */
