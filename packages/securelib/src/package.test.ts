...
// exists to catch before it ever reaches package.json. No exception here,
// unlike @trinoris/securegit's own version of this check — this package has
// nothing internal to depend on beneath it.
...
describe('non-AEAD comparisons use timingSafeEqual', () => {
  ...
  it('no production file outside crypto.ts/envelope.ts/identity.ts/recovery.ts calls Buffer#equals()', async () => {
    ...
    // directly. recovery.ts's one use is parseRecoveryCode's checksum check:
    // even though the recovery code itself is secret, both sides of that
    // comparison are derived from the *same caller-supplied input* — it's a
    // transcription-typo catcher, not a security boundary, and reveals
    // nothing about a secret the caller doesn't already control. The actual
    // secrecy check is `importRecovery`'s AEAD auth tag, timing-safe by
    // construction via Node's own crypto. `@trinoris/securegit`'s own
    // package.test.ts covers the same property for its one exempted file
    // (cli.ts) on that side of the split. Anywhere else, a raw `.equals()`
    // on a Buffer is exactly the mistake `equalCt` exists to prevent (see
    // `keyring.ts`'s fingerprint check, which uses it).
    const allowedBasenames = new Set(['crypto.ts', 'envelope.ts', 'identity.ts', 'recovery.ts']);
    const files = await listProductionSourceFiles(join(REPO_ROOT, 'src'));
    const offenders: string[] = [];
    for (const file of files) {
